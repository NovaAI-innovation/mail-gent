...
**Source:** https://docs.agno.com/reference-api/schema/a2a/run-message-agent.md
**Section:** Docs
...
````yaml post /a2a/agents/{id}/v1/message:send
openapi: 3.1.0
info:
  title: Agno AgentOS
...
paths:
  /a2a/agents/{id}/v1/message:send:
    post:
      ...
      description: >-
        Send a message to an Agno Agent (non-streaming). The Agent is identified
        via the path parameter '{id}'. Optional: Pass user ID via X-User-ID
        header (recommended) or 'userId' in params.message.metadata.
