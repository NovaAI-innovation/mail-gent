...
**Source:** https://docs.agno.com/context/team/overview.md
**Section:** Docs
...
| `member_timeout`                   | `float`     | `None`  | Timeout in seconds for member delegations. Prevents hanging on unresponsive members.                                                                                                                                |
...
| `max_interactions_to_share`        | `int`       | `None`  | Maximum number of member interactions to share when `share_member_interactions=True`. Limits context size.                                                                                                          |
...
<Warning>
  **Configuration Validation**: Setting `delegate_to_all_members=True` and `respond_directly=True` together is not allowed and will raise a `ValueError`. These settings are incompatible because `delegate_to_all_members` requires the team leader to coordinate responses, while `respond_directly` bypasses that coordination.
</Warning>
...
| Parameter                   | Type    | Default | Description                                                                                                        |
| --------------------------- | ------- | ------- | ------------------------------------------------------------------------------------------------------------------ |
| `member_timeout`            | `float` | `None`  | Timeout in seconds for member delegations. Useful for preventing the team from hanging on unresponsive members     |
| `max_interactions_to_share` | `int`   | `None`  | Limits the number of member interactions shared when `share_member_interactions=True` to prevent context explosion |
...
team = Team(
    ...
    member_timeout=30.0,  # 30 second timeout for member responses
    share_member_interactions=True,
    max_interactions_to_share=5,  # Only share the last 5 interactions to limit context size
)
```
...
* [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/04_teams)
