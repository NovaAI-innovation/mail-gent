...
**Source:** https://docs.agno.com/reference-api/schema/a2a/stream-message-team.md
**Section:** Docs
...
````yaml post /a2a/teams/{id}/v1/message:stream
openapi: 3.1.0
info:
  title: Agno AgentOS
...
paths:
  /a2a/teams/{id}/v1/message:stream:
    post:
      ...
      description: >-
        Stream a message to an Agno Team (streaming). The Team is identified via
        the path parameter '{id}'. Optional: Pass user ID via X-User-ID header
        (recommended) or 'userId' in params.message.metadata. Returns real-time
        updates as newline-delimited JSON (NDJSON).
