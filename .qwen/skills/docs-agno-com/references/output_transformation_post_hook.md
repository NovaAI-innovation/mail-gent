...
**Source:** https://docs.agno.com/hooks/usage/team/output-transformation-post-hook.md
**Section:** Docs
...
def add_disclaimer_and_timestamp(run_output: RunOutput) -> None:
    ...
    Useful for teams providing advice or information that needs context.
    """
    content = run_output.content.strip()
...
def structure_financial_advice(run_output: RunOutput) -> None:
    ...
    Uses an AI team to format the response into a structured format
    with key points, disclaimers, and follow-up suggestions.
    """

    # Create a formatting team
    formatter_team = Team(
    ...
    try:
        formatted_result = formatter_team.run(
