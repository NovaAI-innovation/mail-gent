...
**Source:** https://docs.agno.com/cookbook/workflows/employee-recruiter.md
**Section:** Docs
...
The workflow processes candidate resumes through multiple phases with real-time streaming updates. You'll see screening results with scores and feedback, interview scheduling confirmations for qualified candidates (score >= 5.0), and email delivery notifications.

The workflow caches resume content in session state to avoid re-processing. Note that Zoom scheduling and email sending use simulated tools for demonstration - replace with real integrations for production use.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash  theme={null}
    uv pip install -U agno openai fastapi pypdf requests
    ```
  </Step>
...
* Replace simulated Zoom and email tools with real integrations (Zoom API, SendGrid, etc.)
* Modify scoring criteria in the screening agent's instructions
* Adjust the qualifying score threshold (currently 5.0) for interview scheduling
* Explore [Workflows](/workflows/overview) for advanced workflow patterns
