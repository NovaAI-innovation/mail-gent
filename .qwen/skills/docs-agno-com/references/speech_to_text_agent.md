...
**Source:** https://docs.agno.com/cookbook/agents/speech-to-text-agent.md
**Section:** Docs
...
```python speech_to_text_agent.py theme={null}
import httpx
...
# Provide the agent with the audio file and get result as text
agent = Agent(
    model=OpenAIResponses(id="gpt-audio-2025-08-28", modalities=["text"]),
    markdown=True,
    instructions=INSTRUCTIONS,
    output_schema=Transcription,
...
<Steps>
  ...
  <Step title="Install dependencies">
    `bash uv pip install -U agno google-genai httpx `
  </Step>
