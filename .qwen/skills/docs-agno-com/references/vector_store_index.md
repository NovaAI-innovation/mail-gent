# Vector Store Index

**Source:** https://docs.agno.com/knowledge/vector-stores/index.md
**Section:** Docs

**Description:** Index of all vector stores supported by Agno.

---

> ## Documentation Index
> Fetch the complete documentation index at: https://docs.agno.com/llms.txt
> Use this file to discover all available pages before exploring further.

# Vector Store Index

> Index of all vector stores supported by Agno.

Agno supports the following vector database providers organized by category:

### SQL-Based Vector Databases

<CardGroup cols={3}>
  <Card title="PgVector" icon="database" iconType="duotone" href="/knowledge/vector-stores/pgvector/overview">
    PostgreSQL vector similarity search.
  </Card>

  <Card title="SingleStore" icon="database" iconType="duotone" href="/knowledge/vector-stores/singlestore/overview">
    SingleStore distributed SQL with vectors.
  </Card>

  <Card title="ClickHouse" icon="database" iconType="duotone" href="/knowledge/vector-stores/clickhouse/overview">
    ClickHouse OLAP with vector support.
  </Card>
</CardGroup>

### Dedicated Vector Databases

<CardGroup cols={3}>
  <Card title="Pinecone" icon="database" iconType="duotone" href="/knowledge/vector-stores/pinecone/overview">
    Pinecone fully managed vector database.
  </Card>

  <Card title="Qdrant" icon="database" iconType="duotone" href="/knowledge/vector-stores/qdrant/overview">
    Qdrant vector search engine integration.
  </Card>

  <Card title="Weaviate" icon="database" iconType="duotone" href="/knowledge/vector-stores/weaviate/overview">
    Weaviate cloud-native vector database.
  </Card>

  <Card title="Milvus" icon="database" iconType="duotone" href="/knowledge/vector-stores/milvus/overview">
    Milvus open-source vector database.
  </Card>

  <Card title="Chroma" icon="database" iconType="duotone" href="/knowledge/vector-stores/chroma/overview">
    Chroma AI-native embedding database.
  </Card>
</CardGroup>

### NoSQL Vector Databases

<CardGroup cols={3}>
  <Card title="MongoDB" icon="leaf" iconType="duotone" href="/knowledge/vector-stores/mongodb/overview">
    MongoDB Atlas vector search integration.
  </Card>

  <Card title="Azure Cosmos DB MongoDB vCore" icon="microsoft" iconType="duotone" href="/knowledge/vector-stores/azure_cosmos_mongodb/overview">
    Azure Cosmos DB for MongoDB vCore.
  </Card>

  <Card title="Redis" icon="database" iconType="duotone" href="/knowledge/vector-stores/redis/overview">
    Redis vector similarity search.
  </Card>

  <Card title="Couchbase" icon="database" iconType="duotone" href="/knowledge/vector-stores/couchbase/overview">
    Couchbase vector search integration.
  </Card>

  <Card title="Cassandra" icon="database" iconType="duotone" href="/knowledge/vector-stores/cassandra/overview">
    Apache Cassandra vector capabilities.
  </Card>

  <Card title="SurrealDB" icon="database" iconType="duotone" href="/knowledge/vector-stores/surrealdb/overview">
    SurrealDB multi-model with vectors.
  </Card>
</CardGroup>

### Local Vector Databases

<CardGroup cols={3}>
  <Card title="LanceDB" icon="database" iconType="duotone" href="/knowledge/vector-stores/lancedb/overview">
    LanceDB vector database.
  </Card>
</CardGroup>

### Cloud Vector Databases

<CardGroup cols={3}>
  <Card title="Upstash" icon="cloud" iconType="duotone" href="/knowledge/vector-stores/upstash/overview">
    Upstash serverless vector database.
  </Card>
</CardGroup>

### Graph & Specialized Vector Databases

<CardGroup cols={3}>
  <Card title="LightRAG" icon="diagram-project" iconType="duotone" href="/knowledge/vector-stores/lightrag/overview">
    LightRAG graph-based RAG system.
  </Card>
</CardGroup>
